...
const SideMenu: React.FC = () => {
  ...
  const codingPanel = (
    <div className="p-2 flex flex-col gap-2">
      <Button type="primary" icon={<PlusOutlined />} block
        onClick={() => { setSubKey("coding"); refreshCodingSessions(); navigate("/coding"); }}>
        新建 Coding 会话
      </Button>
      <Text type="secondary" className="text-xs">已有 Coding 会话</Text>
      <List size="small" dataSource={codingSessions}
        renderItem={(s: any) => (
          <List.Item className="cursor-pointer hover:bg-gray-50 rounded px-2 group"
            onClick={() => { setSubKey("coding"); refreshCodingSessions(); navigate("/coding/" + s.id); }}
            actions={[
              <Popconfirm key="del" title="删除此 Coding 会话？" description="删除后无法恢复"
                ...
                <Button type="text" size="small" icon={<DeleteOutlined />} danger
                  className="opacity-0 group-hover:opacity-100" onClick={(e) => e.stopPropagation()} />
              </Popconfirm>
            ]}>
  ...
  const items: TabsProps["items"] = [
      ...
      children: (
        <div className="flex flex-col h-full">
          <div className="flex border-b border-gray-200 shrink-0">
            <Button type={subKey === "chat" ? "primary" : "text"} size="small"
              style={{ flex: 1, borderRadius: 0 }} icon={<RobotOutlined />}
              onClick={() => { setSubKey("chat"); navigate("/chat"); }}>
              普通对话
            </Button>
            <Button type={subKey === "coding" ? "primary" : "text"} size="small"
              style={{ flex: 1, borderRadius: 0 }} icon={<CodeOutlined />}
              onClick={() => { setSubKey("coding"); navigate("/coding"); }}>
              AI Coding
            </Button>
          </div>
          <div className="flex-1 min-h-0 h-full overflow-y-auto">
            {subKey === "chat" && chatPanel}
            {subKey === "coding" && codingPanel}
          </div>
        </div>
      ),
    },
    {
      key: "agent",
      label: <span className="select-none"><RobotOutlined /> 智能体</span>,
      children: <div className="px-2"><AgentTabContent
            agents={agents}
            onCreateAgentClick={() => { setEditingAgent(null); setIsAddAgentModalOpen(true); }}
            onSelectAgent={(agentId) => navigate("/chat?agentId=" + agentId)}
            onEditAgent={(agent) => { setEditingAgent(agent); setIsAddAgentModalOpen(true); }}
            onDeleteAgent={(agentId) => deleteAgentHandle(agentId)}
          /></div>,
    },
    {
      key: "knowledgeBase",
