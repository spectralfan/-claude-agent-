...
| MCP 工具 | mcp-proxy + `command-runner` + Shell 策略门禁，按白名单注入 Agent | `mcp.enabled=true`（可关） |
...
| Scheduler/Worker/Reviewer | DAG 编排（PostgreSQL）；并行 Worker + 自动 Reviewer；完成后自动续跑 Scheduler | `coding.subagent.enabled=true` + `coding.orchestration.*` |
